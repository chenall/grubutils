...
#define CM_KEY_FAST_LEAF        0x666c  // "lf"     CM键 快速索引
...
typedef struct
{
    ...
    wchar_t FileName[HIVE_FILENAME_MAXLEN + 1];  //文件名  31宽字符+1结止符
...
typedef struct
{
    ...
    uint32_t FileOffset;                          //文件偏移
    uint32_t Size;                                //尺寸
    uint32_t Reserved[2];                         //保留
    uint64_t TimeStamp;                           //时间戳
    uint32_t Spare;                               //备用
} __attribute__ ((packed)) HBIN;  //HBIN
...
typedef struct
{
    ...
    wchar_t Name[1];                              //名称            NewStoreRoot    12000004        11000001
...
typedef struct
{
    ...
    wchar_t Name[1];                              //名称        KeyName(BCD00000001)  TypeH(320)  Element
