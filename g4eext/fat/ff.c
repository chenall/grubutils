...
#include "fat.h"		/* Declarations of low level disk I/O functions */
...
#if 1
#define mem_cpy(dst,src,cnt) memmove(dst,src,cnt)
...
)
{
	...
	if ((LD_DWORD(&fs->win[BS_FilSysType32]) & 0xFFFFFF) == 0x544146)
		return 0;
	if ((LD_DWORD(&fs->win[3]) & 0xFFFFFFFF) == 0x5346544e) /* Check "NTFS" string */
		return 4;
...
)
{
	...
	*rfs = fs = FatFs[vol];			/* Return pointer to the corresponding file system object */
	...
	if (fmt == 4) return FR_NO_FILESYSTEM;
...
)
{
	...
	if (vol >= _DRIVES)				/* Check if the drive number is valid */
		return FR_INVALID_DRIVE;
  FatFs[0] = zalloc (sizeof(FATFS));;	/* Pointer to the file system objects (logical drives) */
...
)
{
	...
	*bw = 0;	/* Initialize byte counter */
	...
	for ( ;  btw;									/* Repeat until all data transferred */
		buff?(wbuff += wcnt, *bw += wcnt):0, fp->fptr += wcnt, btw -= wcnt) {
		if ((fp->fptr % SS(fp->fs)) == 0) {			/* On the sector boundary? */
			csect = (BYTE)(fp->fptr / SS(fp->fs) & (fp->fs->csize - 1));	/* Sector offset in the cluster */
			if (!csect) {							/* On the cluster boundary? */
			...
			if (!buff) {
				if (cc = btw / SS(fp->fs))
				{
					if (csect + cc > fp->fs->csize) cc = fp->fs->csize - csect;
					wcnt = SS(fp->fs) * cc;
				}
				  else 
				{
